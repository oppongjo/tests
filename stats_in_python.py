...
def get_stats(file_name, column_to_parse):
    numbers = []
    ...
    with open(file_name, 'r') as f:
        for line in f:
            ...
            try:
                fields = line.split('\t')
                num = float(fields[column_to_parse])
                numbers.append(num)
            except (IndexError, ValueError):
                print(f"Skipping line: could not convert to float: {line}")

    if len(numbers) == 0:
        print(f"Error: There were no valid numbers in column {column_to_parse} in file: {file_name}")
        return
